...
```powershell
$u='https://raw.githubusercontent.com/chenzai666/proxy-setup/master/setup_proxy.ps1';$p=Join-Path $env:TEMP ('proxy-setup-'+[guid]::NewGuid().ToString('N')+'.ps1');try{Invoke-WebRequest -UseBasicParsing $u -OutFile $p;& $p}finally{Remove-Item $p -Force -ErrorAction SilentlyContinue}
```

兼容原有命令（仍可执行，但不建议用于新部署）：
...
> 脚本下载到随机临时文件后执行并立即删除，不使用 `iex`。若当前用户未配置执行策略且系统处于默认 `Restricted`，Windows 版会初始化为 `CurrentUser RemoteSigned`，确保新 PowerShell 可加载代理 profile；不会覆盖组策略、LocalMachine 或已明确设置的 `AllSigned` / `Restricted`。
...
```bat
curl -fsSL https://cdn.jsdelivr.net/gh/chenzai666/proxy-setup@master/setup_proxy_cmd.txt -o %TEMP%\setup_proxy.cmd && %TEMP%\setup_proxy.cmd --cdn && del %TEMP%\setup_proxy.cmd
```

`--cdn` 是固定内置 CDN 源；自动化场景仍可使用 `PROXY_SETUP_USE_CDN=1`。
...
```powershell
$u='https://cdn.jsdelivr.net/gh/chenzai666/proxy-setup@master/setup_proxy.ps1';$p=Join-Path $env:TEMP ('proxy-setup-'+[guid]::NewGuid().ToString('N')+'.ps1');try{Invoke-WebRequest -UseBasicParsing $u -OutFile $p;& $p}finally{Remove-Item $p -Force -ErrorAction SilentlyContinue}
```

兼容原有 CDN 命令（仍可执行，但不建议用于新部署）：
...
```bash
curl -fsSL https://cdn.jsdelivr.net/gh/chenzai666/proxy-setup@master/setup_proxy.sh -o /tmp/sp.sh && PROXY_SETUP_USE_CDN=1 bash /tmp/sp.sh; rm -f /tmp/sp.sh
```
...
macOS 版本默认写入 `~/.zshrc`，如需写入 bash 配置可先设置 `PROXY_SETUP_RC_FILE=$HOME/.bash_profile`；不会因为用 `bash /tmp/sp.sh` 执行就误写到 `~/.bashrc`。分发入口只接受内置 GitHub 或 jsDelivr 地址；原有环境变量仅兼容这两个固定地址，不接受任意远程 URL。
...
更新或移除代理配置块时，Python 和 PowerShell 版本会先在同目录创建 `文件名.proxy-bak`，再通过临时文件原子替换原 profile；写入中断时原 profile 保持不变。
...
| `CLAUDE_CODE_INSTALL_URL=URL` | 自定义安装脚本地址，需同时显式授权 | `https://claude.ai/install.ps1` |
| `CLAUDE_CODE_ALLOW_UNSAFE_SOURCE=1` / `-AllowUnsafeInstallSource` | 允许已人工核验的自定义安装源 | 否 |
...
| `CLAUDE_CODE_INSTALL_URL=URL` | 自定义安装脚本地址，需同时显式授权 | `https://claude.ai/install.sh` |
| `CLAUDE_CODE_ALLOW_UNSAFE_SOURCE=1` / `--allow-unsafe-source` | 允许已人工核验的自定义安装源 | 否 |
